...
/**
 * Run photo analysis programmatically (for integration with analyze-all.ts)
 *
 * @param anthropic - Anthropic client instance
 * @param options - Analysis options
 * @returns Object with counts of analyzed photos and any new analyses
 */
export async function runPhotoAnalysis(
  anthropic: Anthropic,
  options: { force?: boolean; verbose?: boolean } = {},
): Promise<{ analyzed: number; skipped: number; total: number }> {
  // Load photos.json
  const photosFile = join(process.cwd(), 'photos.json');
  if (!existsSync(photosFile)) {
    console.log('   ℹ️  No photos.json found, skipping photo analysis');
    return { analyzed: 0, skipped: 0, total: 0 };
  }

  const photosData: PhotosJson = JSON.parse(readFileSync(photosFile, 'utf-8'));

  if (photosData.count === 0) {
    console.log('   ℹ️  No photos to analyze');
    return { analyzed: 0, skipped: 0, total: 0 };
  }
  ...
  if (photosToAnalyze.length === 0) {
    console.log(`   ✅ All ${photosData.count} photos already analyzed`);
    return { analyzed: 0, skipped: photosData.count, total: photosData.count };
  }

  console.log(`   📸 Analyzing ${photosToAnalyze.length} new photos...`);
  ...
  for (const photo of photosToAnalyze) {
    try {
      ...
      console.log(
        `      ${photo.date}: ${analysis.overallScore}/10 - "${analysis.description.slice(0, 40)}..."`,
      );
  ...
  if (successCount > 0) {
    ...
    console.log(`   ✅ Photo analysis saved (${successCount} new, ${errorCount} errors)`);
  }

  return {
    analyzed: successCount,
    skipped: existingPhotos.size,
